...
void loadTexture(const char* file_path) {
    ...
    if (p_surface_image == NULL) {
        SDL_LogError(
            ...
            "loadTexture(): failed to load image to p_surface_image\n%s\n",
            SDL_GetError()
        );
    ...
    SDL_Surface* const p_surface_converted_image = 
        SDL_ConvertSurface(p_surface_image, SDL_PIXELFORMAT_RGBA8888);
    SDL_DestroySurface(p_surface_image);
    if (p_surface_converted_image == NULL) {
        SDL_LogError(
            SDL_LOG_CATEGORY_ERROR,
            "loadTexture(): failed to load image to p_surface_converted_image\n%s\n",
            SDL_GetError()
        );
    ...
    if (p_surface_converted_image->pitch != (int)(texture_width * 4u)) {
        SDL_LogError(SDL_LOG_CATEGORY_ERROR, "loadTexture(): loaded image has invalid pitch\n");
        SDL_DestroySurface(p_surface_converted_image);
        exit(EXIT_FAILURE);
    }
    ...
    if (texture_width & texture_height) {
        ...
        SDL_LogError(SDL_LOG_CATEGORY_ERROR, "loadTexture(): individual image isn't square\n");
        SDL_DestroySurface(p_surface_converted_image);
        exit(EXIT_FAILURE);
    ...
    size_t image_width, image_height = 0u;
    
    for (; image_height < texture_height; image_height++) {
        for (image_width = 0u; image_width < texture_width; image_width++) {
